...
pub fn command_impl(args: CommandArgs, func: ItemFn) -> TokenStream {
            ...
            FnArg::Typed(typed) => {
                ...
                if let Some(inner_type) = dependency_injection::extract_depend_inner_type(ty) {
                    ...
                    param_extractions.push(quote! {
                        let #arg_name: #ty = <#ty as dorimubot_framework::FromCommonMessage<'_>>::from(__message);
                    });
    ...
    quote! {
        ...
        const _: () = {
            ...
            fn #wrapper_name<'a>(
                __message: &'a dyn dorimubot_framework::CommonMessage,
                __dependencies: &'a dyn dorimubot_framework::DependencyProvider,
            ) -> dorimubot_framework::CommandHandleFuture<'a> {
                ::std::boxed::Box::pin(async move {
                    #(#param_extractions)*
                    #invoke
                    // 将结果转换为统一的输出格式
                    dorimubot_framework::CommandOutput::into_output(result)
                })
            }

            // 使用 inventory 注册命令
            dorimubot_framework::inventory::submit! {
                dorimubot_framework::CommandDef {
                    prefix: #prefix,

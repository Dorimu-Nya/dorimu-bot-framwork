...
pub fn qqbot_config() -> QQBotConfig {
    let base_url = mock_api_base_url();
...
#[allow(dead_code)]
pub fn qqbot_config_without_bot_info() -> QQBotConfig {
    let api_url = format!("{}/without-bot-info", mock_api_base_url());
    ...
    QQBotConfig::new()
        ...
        .api_override(&api_url)
}

fn mock_api_base_url() -> &'static str {
    MOCK_API_BASE_URL.get_or_init(|| {
...
fn handle_request(mut stream: TcpStream) {
    ...
    let (status, response_body) = match path {
        ...
        "/without-bot-info/users/@me" => (
            "401 Unauthorized",
            r#"{"id":"ignored-bot-id","username":"ignored-bot","bot":true}"#,
        ),

...
use super::store::CommandsStore;
use crate::app::config::AppConfig;
use crate::app::{ApiClient, App, ContextStore};
...
use crate::events::group::event_type::GroupEventTypeKind;
use crate::events::group::models::GroupAtMessage;
#[cfg(feature = "macros")]
...
impl CommandPlugin {
    ...
    pub(crate) fn from_config(config: &AppConfig, app: &App) -> Self {
        ...
        for command in inventory::iter::<CommandDef> {
            let replaced = commands.insert(
                command.prefix,
                super::defining::wrap_command_handle_fn(command.handler),
            );
